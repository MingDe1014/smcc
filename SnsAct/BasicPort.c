...
void BasicPort_init(void)
{
	...
	IfxPort_setPinModeInput(SWITCH_NORM.port, SWITCH_NORM.pinIndex, IfxPort_InputMode_noPullDevice);	// 정규화 스취치
	IfxPort_setPinModeInput(MODE_1.port, MODE_1.pinIndex, IfxPort_InputMode_noPullDevice);
	IfxPort_setPinModeInput(MODE_2.port, MODE_2.pinIndex, IfxPort_InputMode_noPullDevice);	// 정규화 스취치
	...
	IR_Port.mode1 = IfxPort_getPinState(MODE_1.port, MODE_1.pinIndex);
	IR_Port.mode2 = IfxPort_getPinState(MODE_2.port, MODE_2.pinIndex);
	IR_Port.switch_norm =  IfxPort_getPinState(SWITCH_NORM.port, SWITCH_NORM.pinIndex);
...
uint8 Switch_State(void){
	//on 0, off 1
	if(IR_Port.switch_norm != FALSE){
		return 1;
	}
